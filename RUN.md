...
10. **Slackに今日の成果を報告**（#mona-日報 = `C0BJ6E29GKA` へ `slack_send_message`。
   失敗した場合のみ Ryota（`ryota4100221`）宛のDM）。
   ⚠️ **mona Webhook は使わない**（2026-07-26に手順から削除）。クラウド実行環境は
   `hooks.slack.com` を403で拒否することが確定していて（OPERATIONS.md §2 の egress 制限）
   常に無駄打ちになるうえ、Webhook URL をルーティン設定の本文に置くと共有・
   エクスポート時に漏れる。この行は Day 069 まで旧記述のまま残っていた。
   フォーマット:
   ```
   🎨 今日の3D — Day NNN
   再現元: <サイト名> / <URL>
