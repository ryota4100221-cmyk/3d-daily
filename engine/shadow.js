...
import { MASS_KAPPA, TILT_GAIN } from './footprint.js'
import { PANE_RECTS } from './glass.js'

/**
 * Day 047 — the shadow test, asked *where*.
 *
 * Day 039 turned this file's answer from a scalar into six numbers, one per
 * pane, and every day since has spent them the same way: as weights on the six
 * polygons of the LTC sum. That is the visible *mass* and it is not the whole of
 * what the split-sum needs. The other factor — the mean radiance the lobe reads
 * out of the pyramid — has never had any visibility in it at all, so a blocker
 * covering the warm light of this window has been making the room darker without
 * making it greener.
 *
 * So the same forty-eight taps now also report **where** the light stops. Their
 * hits are least-squared against a plane in the window's own uv, which is the
 * cheapest description that can hold the thing that is actually there: the
 * silhouette of a straight blocker seen from a point is a straight line on the
 * light's plane, at any angle, anywhere, mullions or no mullions. Three floats
 * come back beside the six; `glassFetch` premultiplies each of its taps by them.
 *
 * The derivation, and why three numbers rather than six or a texture, is in
 * `visfit.js`. What is here is the accumulation, and it costs six adds a tap.
 *
 * ---------------------------------------------------------------------------
 * Day 041's header follows.
 *
 * The shadow test, reading the colour off the picture.
 *
 * Yesterday the taps started carrying a colour: `srcSample` walked a CDF, landed
 * in a pane, and handed back that pane's constant along with the offset. Today
 * the panes have no constants — the radiance is an image — so the tap does the
 * one thing it is uniquely placed to do and **looks the colour up where it
 * landed.** It already has the coordinate; it is the offset it was about to
 * return. `uPaneCol` is not consulted, and the CDF walk keeps its other job of
 * distributing taps by area.
 *
 * The level it reads at is not zero, and that is a deliberate variance trade
 * rather than an oversight. A froxel spends two taps and a mote spends one. At
 * level 0 a single tap is a point sample of a painted window and the air would
 * boil with the quarries; the phase function is near-constant over thirteen
 * degrees, so what the air actually wants is the *mean over the visible part*,
 * and reading each tap pre-averaged over a small footprint is an estimator of
 * exactly that with a fraction of the variance. Two taps at level 3 see 128
 * texels of window between them. Bias for variance, at the one place in the
 * renderer where the sample budget is a hundredth of the surfaces'.
 *
 * ---------------------------------------------------------------------------
 * Day 040's header follows.
 *
 * The shadow test, asked *what colour*.
 *
...
/**
 * Day 047 — the same six rectangles, in the *image's* uv rather than in angle.
 *
 * A tap is drawn inside a pane as a fraction of that pane's own rectangle, so
 * the cheapest way to put it on the picture is to interpolate the pane's corners
 * in the picture's frame: one mix, no tan() per tap. `PANE_RECTS` is already in
 * the fetch's own coordinates — the window's *plane*, through tan, [-1, 1] — so
 * this is the same frame `glassFetch` computes its uv in, which is the whole
 * requirement. The residual is that a tap's position *within* a pane is linear
 * in angle where the plane is linear in tan; over four degrees that is under a
 * tenth of a per cent, and it lands on a fit that is itself a plane.
 */
function padPaneUV(rects) {
  const out = []
  for (let i = 0; i < MAX_PANES; i++) {
    const p = rects[i] || { u0: 0, u1: 1, v0: 0, v1: 1 }
    // [-1, 1] over the bounding rectangle -> [0, 1] over the image.
    out.push(
      new THREE.Vector4(
        0.5 + 0.5 * p.u0,
        0.5 + 0.5 * p.v0,
...
export function shadowUniforms() {
  return {
    ...
    // Day 047: the same six rectangles in the image's uv, so a tap can say where
    // on the *picture* it landed without a tan() and without knowing that the
    // window has a frame around it.
    uPaneUV: { value: WINDOW_DATA.paneUV },
    // Day 047, and it ships at **0**, which is the whole of what the morning
    // found out. 1 weights each tap of the fetch by the visibility the mask
    // fitted; it is one multiply, it is arithmetically sound, and measured over
    // 1022 occluded configurations it is worth -0.37 points, i.e. it loses.
    // The reason is in NOTES and it is not a tuning problem: the mean tap count
    // is 1.31, and at one tap a scalar multiplies the colour and the coverage
    // alike, so the divide cancels it exactly. It is kept, off, because a
    // negative result nobody can re-run is an assertion.
    uVisFit: { value: 0 },
    ...
    // --- the glass as an image (Day 041), under a ripmap (Day 042) ------------
    //
    // Declared here rather than in ltc.js because this is the file that owns the
    // description of the source, and all three consumers of that description —
    // the composite, the froxels, the motes — read the picture. The integral
    // reads it at a *pair* of levels it computes; the air and the dust read the
    // square cell (uGlassSoft, uGlassSoft), which is bit for bit the mip level
    // they read yesterday because the mip chain is the ripmap's diagonal.
    uGlass: { value: null },
    uGlassMax: { value: 8 },
    uGlassK: { value: 256.0 },
    uGlassSoft: { value: 3.0 },
    // Atlas width in texels (2N) and base image width (N) — the two numbers
    // `ripCell` needs to turn a level pair into an address.
    uRipAtlas: { value: 512.0 },
    uRipBase: { value: 256.0 },
    // 0 puts Day 041's scalar footprint back — one level, on the ripmap's
    // diagonal, which is the square pyramid reading the same memory. Not a
    // second code path: the identical fetch at the identical coordinates, with
    // the elliptical region replaced by the round one.
    uAniso: { value: 1 },
    // Day 043's budget: the most taps the fetch may take along the ellipse's
    // major axis. 1 collapses it to Day 042 — one sub-ellipse of half-length A,
    // which is the whole ellipse's bounding box again — so ?tap=1 reads the same
    // cells of the same atlas as yesterday, and the difference between the two
    // renders is only where the ripmap was asked to look.
    uTapMax: { value: 4 },
    // Day 044: the source's own support, as a semi-axis in texels. The kernel
    // the fetch stands in for is the lobe *times the window*, and the window is
    // a unit square whose covariance is (1/12) I — an ellipse of N/sqrt(3). The
    // two variances add reciprocally, so this shortens both semi-axes and turns
    // nothing. 0 removes the day and restores Day 043's ellipse exactly.
    uSupport: { value: 256.0 / Math.sqrt(3) },
    // Day 045: the profile constant that turns the kernel's mass over its peak
    // into an area — 1 if the kernel were uniform on its ellipse, 2 if it were a
    // Gaussian of the same covariance. Measured at 1.2 against 1024 brute-forced
    // configurations; see scripts/lobe-error.mjs. 0 removes the day and leaves
    // Day 044's two constants deciding the size.
    uMass: { value: MASS_KAPPA },
    // Day 048: the reliability the tilt is applied at. The derivation predicts
    // the whole displacement; measured against the kernel's true first moment it
    // is right in direction (R 0.62) and about 1.6x too long, and both harnesses
    // sweep to the same 0.5. 0 removes the day and puts the fetch back on
    // Day 041's ray-plane intersection.
    uTilt: { value: TILT_GAIN },
    // 0 falls back to Day 040's six constants with everything else held.
    uTexLight: { value: 1 },
...
export const SHADOW_GLSL = /* glsl */ `
  ...
  uniform vec4 uPaneUV[LTC_PANES];
  uniform float uPaneCdf[LTC_PANES];
  uniform vec3 uPaneCol[LTC_PANES];
  uniform float uTint;
  uniform float uVisFit;
  uniform sampler2D uGlass;
  uniform float uGlassMax;
  uniform float uGlassK;
  uniform float uGlassSoft;
  uniform float uRipAtlas;
  uniform float uRipBase;
  uniform float uAniso;
  uniform float uTapMax;
  uniform float uSupport;
  uniform float uMass;
  uniform float uTilt;
  uniform float uTexLight;
  uniform int uPaneN;
  ...
  /**
   * One cell of the ripmap: the window averaged over a 2^m by 2^n footprint.
   *
   * Two multiply-adds and a clamp. The clamp is the price of an atlas — hardware
   * bilinear will happily walk out of cell (m, n) and into cell (m, n+1), which
   * are neighbours in memory and unrelated in content, so the uv is inset by half
   * a texel of *this* cell before it is placed. Insetting rather than padding
   * because a padded ripmap is 25% larger and the inset costs one vec2 madd.
   *
   * exp2 of a float rather than a shift: the level pair arrives as the two halves
   * of a lerp and is fractional right up until floor() is called on it, and this
   * function is the thing floor() is called *for*.
   */
  vec4 ripCell(vec2 uv, vec2 lv) {
    vec2 sz  = vec2(uRipBase) / exp2(lv);                 // cell size, texels
    vec2 org = uRipAtlas - uRipAtlas / exp2(lv);          // cell origin, texels
    vec2 t   = clamp(uv, 0.5 / sz, 1.0 - 0.5 / sz) * sz + org;
    // Day 046: rgba, not rgb. The alpha channel carries the pane coverage,
    // filtered by the same two box passes as the colour, and every caller
    // divides — see glassMean below.
    return TEXLOD(uGlass, t / uRipAtlas, 0.0);
  }

  /**
   * Day 046 — premultiplied out. The mean of the glass alone over whatever
   * footprint the caller averaged.
   *
   * Level 0 holds (L.m, m) with m the aperture's coverage, and a box filter is
   * linear, so any cell holds (mean of L.m, mean of m) and their ratio is the
   * mean of L over the glass inside that cell. One divide, and the mullions stop
   * being averaged into the colour they are not part of.
   *
   * The guard is not a fudge. A footprint entirely inside a mullion has no glass
   * in it and therefore no answer, and that set is real at level 0, where the
   * stone is fifteen texels wide. White is the right thing to return, because
   * white is the whole window's coverage-weighted mean by construction — the
   * pyramid is normalised to it — so this is the same answer the top of the
   * pyramid gives, which is where a fetch with nothing to look at belongs. The
   * blend runs over the first 2% of coverage so no surface steps as it crosses.
   */
  vec3 glassMean(vec4 s) {
    return mix(vec3(1.0), s.rgb / max(s.a, 1e-4), smoothstep(0.0, 0.02, s.a));
  }

  /**
   * The window, averaged over an axis-aligned footprint of 2^lod.x by 2^lod.y
   * texels — with the level pair interpolated, which is the whole reason this is
   * four fetches and not one.
   *
   * Hardware trilinear did this for free yesterday because a mip chain is one
   * dimensional and the sampler owns it. A ripmap is a 2D family and no sampler
   * knows about it, so the bilinear across (m, n) is written out. Skipping it is
   * not an option: the levels are computed per pixel from a Jacobian that varies
   * smoothly across a curved surface, and quantising them to integers puts a
   * visible contour on every sphere in the frame wherever the footprint crosses a
   * power of two. Four bilinear fetches of a 2 MB texture, twice per pixel.
   */
  vec4 ripFetch(vec2 uv, vec2 lod) {
    vec2 l0 = floor(lod);
    vec2 l1 = min(l0 + 1.0, vec2(uGlassMax));
    vec2 f  = lod - l0;
    vec4 a = ripCell(uv, vec2(l0.x, l0.y));
    vec4 b = ripCell(uv, vec2(l1.x, l0.y));
    vec4 c = ripCell(uv, vec2(l0.x, l1.y));
    vec4 d = ripCell(uv, vec2(l1.x, l1.y));
    // The blend across the level pair is linear and so is the premultiplication,
    // so colour and coverage may be carried through it together and divided once
    // at the end. Dividing inside each of the four would be four divides and a
    // *different* answer — a mean of ratios rather than a ratio of means, which
    // is the mistake premultiplied alpha exists to prevent.
    return mix(mix(a, b, f.x), mix(c, d, f.x), f.y);
  }

  /**
   * The window's own image, at an angular offset in its unrotated frame.
   *
   * The image spans the *bounding* rectangle — stone included, drawn through the
   * leads — so the map from a tap's offset to a texel is one multiply-add and
   * needs to know nothing about the panes. Read at the square cell
   * (uGlassSoft, uGlassSoft): see the note at the top of this file about what one
   * tap of a painted window does to a froxel.
   *
   * uGlassSoft is an integer, so this is a single bilinear fetch — the same one
   * cost the air paid yesterday, of the same 32x32 texels. The mip chain is the
   * ripmap's diagonal, so nothing about the volume moved today, by construction
   * rather than by care. A tap of a shadow ray has no lobe and therefore no
   * ellipse; anisotropy is a property of the thing doing the looking.
   */
  vec3 glassAt(vec2 s) {
    vec2 uv = s / max(uSrcBound, vec2(1e-5)) * 0.5 + 0.5;
    // Day 046: one divide, and it is the *same* divide the integral does — a tap
    // of a shadow ray has already chosen a pane by area, so what it wants from
    // the picture is the glass's colour there and never the mullion's share of
    // it. At cell (3, 3) the coverage is close to 1 almost everywhere and this
    // moves the volume by very little, which is the point: the correction is
    // largest exactly where the footprint is largest.
    return glassMean(ripCell(clamp(uv, 0.0, 1.0), vec2(uGlassSoft)));
  }
  ...
  vec2 srcSample(float s, float v, out vec3 col) {
    ...
    for (int i = 0; i < LTC_PANES; i++) {
      ...
      if (s < hi || i == uPaneN - 1) {
        ...
        vec2 off = q.xy + (vec2(u, v) * 2.0 - 1.0) * q.zw;
        // Where it landed *is* the texture coordinate. Yesterday this line read
        // a constant indexed by the pane; the walk located a point and then
        // rounded it to the pane it was in. The image does not need rounding.
        vec3 c = uTexLight > 0.5 ? glassAt(off) : uPaneCol[i];
        col = mix(vec3(1.0), c, uTint);
        return off;
      }
      lo = hi;
    }
  ...
  /**
   * The fitted visibility at a point of the picture, clamped — a visibility is a
   * fraction, and a plane through binary data leaves the interval as soon as it
   * is extrapolated past the edge of the taps that made it.
   *
   * uVisFit = 0 returns 1 everywhere, which makes every weight below the
   * identity and every fetch Day 046's, to the bit.
   */
  float visRaw(vec3 fit, vec2 uv) {
    return clamp(fit.x + dot(fit.yz, uv - vec2(0.5)), 0.0, 1.0);
  }

  /**
   * The same value, as a *weight*. Zero on uVisFit is the shipped build and is
   * the identity — one multiply by one — so the fetch is Day 046's to the bit
   * while the read-out above still draws the pixels the morning was about.
   */
  float visAt(vec3 fit, vec2 uv) {
    return uVisFit < 0.5 ? 1.0 : visRaw(fit, uv);
  }
#ifdef SH_MASK
  /**
   ...
   */
  float softVisMask(vec3 p, vec2 off, vec2 slope, out float visP[LTC_PANES], out vec3 fit) {
    for (int i = 0; i < LTC_PANES; i++) visP[i] = 1.0;
    // Day 047. The plane the taps will be fitted to, initialised to the answer
    // every early-out below is entitled to: fully clear, no gradient. Most of a
    // lit frame leaves through one of them, so the day costs those pixels one
    // vec3 write and nothing else.
    fit = vec3(1.0, 0.0, 0.0);
    ...
    // special case. The plane is that number too, flat: a source with no size
    // has no *where* for a gradient to point along.
    if (uHard > 0.5) {
    ...
    // --- 3. PCF, pane by pane ------------------------------------------------
    //
    // Day 047 adds a second reading of the same taps. Each one already knows two
    // things it has never been asked for: *where on the picture it looked* and
    // whether it got through. Summed as five moments those are a least-squares
    // plane over the window — the description glassFetch needs to weight its
    // own taps by, and the one description a per-pane vector cannot give it,
    // because a blocker's edge does not stop at a mullion.
    int tp = max(SH_PCF / max(uPaneN, 1), 1);
    float vis = 0.0;
    float lo = 0.0;
    // The moments, in the image's uv. Centred after the loop rather than during
    // it — one pass, and the centring is what separates the constant term from
    // the gradient. See visfit.js.
    float n = 0.0;
    vec2 sm = vec2(0.0);      // Σ(u, v)
    vec3 sq = vec3(0.0);      // Σ(u², uv, v²)
    float sh = 0.0;           // Σ hit
    vec2 shp = vec2(0.0);     // Σ hit·(u, v)
    for (int i = 0; i < LTC_PANES; i++) {
      if (i >= uPaneN) break;
      vec4 q = uPane[i];
      vec4 r = uPaneUV[i];
      ...
      for (int j = 0; j < SH_PCF; j++) {
        ...
        // The same tap, on the picture. st is its position inside the pane's
        // own rectangle, so mixing the pane's uv corners places it — no tan, no
        // knowledge of the frame, one mix.
        vec2 t = mix(r.xy, r.zw, st);
        n   += 1.0;
        sm  += t;
        sq  += vec3(t.x * t.x, t.x * t.y, t.y * t.y);
        sh  += hit;
        shp += hit * t;
      }
    ...
    // The plane, from the moments. Centred design: the constant term is the mean
    // hit and the gradient solves a symmetric 2x2, which is three multiplies and
    // one reciprocal.
    if (n > 2.5) {
      vec2 m = sm / n;
      float mh = sh / n;
      float cuu = sq.x / n - m.x * m.x;
      float cuv = sq.y / n - m.x * m.y;
      float cvv = sq.z / n - m.y * m.y;
      vec2 ch = shp / n - mh * m;
      float det = cuu * cvv - cuv * cuv;
      // Collinear taps — a degenerate opening in the comparison renders — leave
      // the gradient at zero rather than at infinity. The scale here is uv
      // squared, so 1e-9 is six orders below any real design.
      vec2 k = abs(det) > 1e-9
        ? vec2(cvv * ch.x - cuv * ch.y, cuu * ch.y - cuv * ch.x) / det
        : vec2(0.0);
      fit = vec3(mh + dot(k, vec2(0.5) - m), k);
    } else {
      fit = vec3(vis, 0.0, 0.0);
    }
    return vis;
  }
