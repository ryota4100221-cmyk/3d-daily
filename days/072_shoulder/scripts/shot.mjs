...
// 🔴 今日の閾値（Day 072）。地はほぼ白で、映るのは布2枚とハンガー2本だけ。
// **実描画は 416〜444KB**。3D が死んで DOM だけ残った場合は白地＋文字になり、
// 実測で **372KB**（budget 16000/18000 で2回出た）。差が 45KB しかないので
// バイト数だけでは足りない——撮った1枚は必ず目で見ること。400KB に置いた。
// 前日の 380KB のままだと 372KB の空カンバスを1回だけ通しかけた。
const MIN_BYTES = Number(process.env.SHOT_MIN || 400) * 1024
