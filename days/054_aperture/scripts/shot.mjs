...
// 🔴 今日の閾値は「空カンバスは小さい」では決められない。DOM に粒子オーバーレイを
// 敷いているので、canvas が1画素も描けていなくても PNG は 400KB 出る（実際に1回引いた）。
// 実描画は 1.4〜1.6MB なので、閾値はその間の 700KB に置く。
// 教訓: 空カンバス判定のバイト閾値は、canvas 以外の高周波成分を足した日には必ず貼り直す。
const MIN_BYTES = Number(process.env.SHOT_MIN || 700) * 1024
