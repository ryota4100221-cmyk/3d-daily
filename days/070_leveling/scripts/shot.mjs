...
// 🔴 今日の閾値（Day 070）。地は白で、その上に黒い見本板が12枚。DOM側に
// 見出しと本文と目盛りが載るので、**空カンバスでも 69KB 出る**（実測、3回とも
// ぴったり 69KB）。実描画は 111〜127KB。前日の 200KB のままだと実描画を弾き、
// 60KB だと空カンバスを通す。95KB に置いた。
//
// 空になる頻度は budget=18000 でも 3回に2回。バイト数のリトライ梯子が
// 無いと日によって黙って空が出る（Day 018/024/026/027 と同じ症状）。
const MIN_BYTES = Number(process.env.SHOT_MIN || 95) * 1024
...
const BUDGETS = process.env.SHOT_BUDGET
  ...
  : [16000, 18000, 20000, 22000, 24000, 18000]
