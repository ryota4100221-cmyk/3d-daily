...
export function shadowUniforms() {
  return {
    ...
    // the composite, the froxels, the motes — now read the picture. The integral
    // reads it at a level it computes; the air and the dust read it at uGlassSoft.
    uGlass: { value: null },
    uGlassMax: { value: 8 },
    uGlassK: { value: 256.0 },
...
export const SHADOW_GLSL = /* glsl */ `
   ...
   * needs to know nothing about the panes. Read at uGlassSoft rather than at
   * level 0: see the note at the top of this file about what one tap of a painted
   * window does to a froxel.
   */
  vec3 glassAt(vec2 s) {
    vec2 uv = s / max(uSrcBound, vec2(1e-5)) * 0.5 + 0.5;
    return TEXLOD(uGlass, clamp(uv, 0.0, 1.0), uGlassSoft).rgb;
  }
