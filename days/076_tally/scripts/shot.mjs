...
// 🔴 今日の閾値（Day 074）。地は #000000 のベタで、映るのは 5,517 本の棒だけ。
// 棒の断面が 5px 程度なので細かいノイズがびっしり乗り、PNG は圧縮しづらい——
// **実描画は 190〜270KB／空カンバスは 30KB**。6倍以上離れたので 120KB に置いた。
// （この日も RUN.md の言う空カンバス誤診が実際に出た。budget=12000 の1回目が
//  30KB で、同じビルド・同じ budget=9000 が 198KB。梯子は飾りではない。）
const MIN_BYTES = Number(process.env.SHOT_MIN || 120) * 1024
