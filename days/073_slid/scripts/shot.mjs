...
// 🔴 今日の閾値（Day 073）。地は #85AFD8 のベタ1面で、映るのは白い角柱15本と
// その影だけ——**PNG にとっては理想的な圧縮対象**で、粒子（紙目）を乗せていなければ
// 実描画でも数十KBまで落ちる。つまりこの日は「バイト数が小さい＝空カンバス」が
// 成り立たない構図だった。地に ±0.0125 の紙目を1枚敷いてあるのはそのためでもある
// （見た目のためだけではない）。**実描画は 520〜640KB／空カンバスは 47KB**で
// 10倍以上離れたので、300KB に置いた。
const MIN_BYTES = Number(process.env.SHOT_MIN || 300) * 1024
