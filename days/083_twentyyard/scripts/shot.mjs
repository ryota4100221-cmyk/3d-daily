...
// 🔴 今日の閾値（Day 083）。実測：3Dが出た＝318〜480KB／出ていない（DOMの
// 文字だけ）＝86KB。閾値は前日から引き継がず、毎日その日の空カンバスと実描画を
// 両方測って置く。
//
// 🔴 今日の別の発見：**budget を伸ばすほど空カンバスになった。**
// 16000〜24000 ms は6回連続で 86KB（＝空）、9000〜12000 ms では出る。
// virtual time の下では rAF が実時間を無視して回り続けるので、budget を伸ばすと
// swiftshader 側が先に息切れして、最後の合成が空になる。
// 「出なければ伸ばす」という前日までの階段は、今日に限っては逆効果だった。
const MIN_BYTES = Number(process.env.SHOT_MIN || 190) * 1024
...
const BUDGETS = process.env.SHOT_BUDGET
  ...
  : [9000, 11000, 8000, 12000, 10000, 9000]
