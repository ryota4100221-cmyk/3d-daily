...
/**
 * Day 036 — the froxel volume, and what a shaft's edge is made of.
 *
 * The grid itself does not change today. Injection still writes a rate and an
 * extinction per cell, the scan still integrates them along the eye ray, the
 * diffusion (Day 033) and the return path (Day 035) still run inside the same
 * one-Jacobi-iteration-per-frame the temporal filter has been accumulating for
 * three days.
 *
 * What changes is the single line at the bottom of all of it: `lightVis(p)`.
 *
 * Since Day 031 that has been `step(d - bias, m)` — one texture fetch, one
 * comparison, a yes or a no. It is why every shaft in this series has had a
 * *ruled* edge. A real shaft does not: the slot that cuts it is being crossed by
 * a source with a width, so the boundary between lit air and unlit air is a
 * gradient whose own width grows with the distance from the slot. Eleven metres
 * downstream of the aperture that gradient is nearly twenty centimetres across,
 * and the beam has been drawing it as a knife edge.
 *
 * The soft test lives in shadow.js and is shared with the surfaces, which is the
 * point — one model, one map, one set of numbers in metres. The difference is
 * what each customer can afford. The composite runs it at 8 search taps and 20
 * PCF taps once per screen pixel. Here it runs at 4 and 2, per cell, 2.36
 * million times, which produces an estimate so coarse that a single frame of it
 * is visibly speckled — and then hands it to a temporal filter that has been
 * integrating four-tap estimates since Day 033. The shaft's edge is soft *over
 * frames*. Nothing else about this pass had to change to allow that; the machine
 * for it was already here, built for something else.
 */
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        vec3 S = uLightCol * (uScatter * sig * softVis(p, rot) * phaseHG(dot(dirW, uLightDir), uG));
