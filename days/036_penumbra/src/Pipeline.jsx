...
/**
 * Eight stages yesterday, seven today, and R3F still issues none of them.
 *
 *   0. Light depth   the scene from the light, as linear distance (Day 031).
 *                    From this morning it is the *only* shadow map in the
 *                    renderer: three's is gone with the light that owned it.
 ...
 *
 * The stage that left is the beauty pass: "render the scene with its own
 * materials and let three light it". It had been in this file, in one form or
 * another, since Day 001. Removing it takes out a full scene draw and a second
 * shadow map, and it is what lets stage 4 hold the direct term, the wash and the
 * return path in one shader where they can be compared.
 */
...
// mode indices of the four comparisons in App's PASSES list. They exist so the
// piece can put its own claims next to the picture without them.
const MODE_DIRECT = 1
const MODE_WASH = 6
...
export default function Pipeline({ rig, mode = 0, freeze = null, phase = 0 }) {
  ...
  useFrame((st, delta) => {
    ...
    // The subject lies on the ground, so the camera goes back up above eye level
    // and tilts down — the opposite of yesterday, which had to see the bottom of
    // a slab. A shadow read at a grazing angle is a line; read from above it is
    // a shape with a gradient at its far end.
    const px = st.pointer.x
    const py = st.pointer.y
    const tx = 1.10 + px * 0.46 + Math.sin(t * 0.067) * 0.26
    const ty = 3.05 + py * 0.26 + Math.sin(t * 0.051 + 1.3) * 0.08
    const tz = 11.9 + Math.cos(t * 0.041) * 0.34
    if (freeze != null) {
    ...
    camera.lookAt(2.35 + px * 0.08, 0.95 + py * 0.05, -0.85)
    ...
    injMat.uniforms.uHard.value = hard
    compMat.uniforms.uHard.value = hard
    rig.moteMat.uniforms.uHard.value = hard
