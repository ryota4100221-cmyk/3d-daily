...
/**
 * Day 036 — the rest of the pipeline, one pass shorter than yesterday.
 *
 *   makeLightDepthMaterial  pass 0: the scene from the light, as *linear*
 *                           distance. Day 031's, unchanged — and today it stops
 *                           being the renderer's second shadow map and becomes
 *                           its only one.
 *   makeGBufferMaterial     pass 1: MRT, three attachments — normal + depth,
 *                           velocity + id, albedo + roughness. Roughness is new,
 *                           and so is the emissive variant that evaluates the
 *                           backdrop's gradient here instead of in a pass of
 *                           its own.
 *   makeCompositeMaterial   pass 2, and today it does the whole of the lighting:
 *                           the direct term (Lambert + GGX, with the soft shadow
 *                           test), the wash out of the volume, the volume along
 *                           the eye ray, and the radiosity attachment the froxel
 *                           injection reads next frame.
 *   makeMoteMaterial        pass 3: 7,000 billboards, now shadowed by the same
 *                           soft test at one tap apiece.
 ...
 *
 * What is *not* here any more is the beauty pass. For thirty-five days this file
 * had a stage that said "render the scene with its own materials and let three
 * light it", and everything downstream treated that image as an input it could
 * not see inside. Deleting it removes a full scene draw, removes the second
 * shadow map, and — the part that matters — puts the direct term, the wash and
 * the return path in one shader, in one set of units, where they can be compared.
 */
...
/**
 * Two things arrive today.
 *
 * Roughness, into the alpha of the albedo attachment, which was carrying a 1.0.
 * Yesterday nobody downstream needed it because three was the one evaluating a
 * BRDF; today the composite is, and a shader that cannot tell limewash from
 * unpolished stone will shade them the same.
 *
 * And an EMISSIVE variant. The backdrop used to be drawn by the beauty pass with
 * a shader material of its own, and the beauty pass no longer exists — so the
 * gradient is evaluated here, written as albedo, and marked in the material id.
 * The composite hands it straight out without lighting it. It costs nothing: the
 * mesh was already in this pass anyway, writing a depth and a velocity that the
 * temporal resolve needs.
 */
...
/**
 * This pass has grown a term every day for a week, and today it grows the one
 * that was always underneath the others.
 *
 *   DIRECT.  Lambert plus one GGX lobe, from the G-buffer's albedo, roughness
 *            and normal, with `softVis` for the shadow — the day's technique.
 *            Until this morning this term came out of a separate full scene draw
 *            shaded by three against three's own shadow map, and everything here
 *            treated it as an opaque input. Now it is four lines above the wash,
 *            in the same units as the wash, reading the same depth map the air
 *            reads.
 *
 ...
 *
 * The thing worth noticing is that `direct + wash` is now a sum of two terms this
 * file computed six lines apart, rather than a foreign image plus a local one.
 * Every knob that used to be "how much of three's output do I trust" is gone.
 */
export function makeCompositeMaterial() {
  return new THREE.ShaderMaterial({
    glslVersion: THREE.GLSL3,
    defines: { SH_SEARCH: 8, SH_PCF: 20 },
    uniforms: {
      ...froxelReadUniforms(),
      ...shadowUniforms(),
      uGbuf: { value: null },
    ...
    fragmentShader: /* glsl */ `
      ...
      /**
       * The direct term. One lamp, one lobe, and a shadow test that returns a
       * fraction.
       *
       * The normal offset before the test is 3.5cm — small, and it is allowed to
       * be small precisely because the shadow is soft now. A hard PCF has to be
       * pushed off the surface far enough to clear its own quantisation *and*
       * far enough that the step function does not shimmer along a grazing
       * plane; a 20-tap average over a real penumbra has no step function to
       * shimmer.
       */
      vec3 direct(vec3 p, vec3 n, vec3 albedo, float rough, float rot) {
        ...
        float a2 = a * a;
        float dnom = ndh * ndh * (a2 - 1.0) + 1.0;
        float D = a2 / (PI * dnom * dnom);
        float k = a * 0.5;
        float G = (ndl / (ndl * (1.0 - k) + k)) * (ndv / (ndv * (1.0 - k) + k));
        float F = uF0 + (1.0 - uF0) * pow(1.0 - vdh, 5.0);
        float spec = D * G * F / max(4.0 * ndl * ndv, 1e-4);

        vec3 brdf = albedo * (1.0 / PI) + vec3(spec * uSpec);
        return uLightCol * (uLightInt * ndl * vis) * brdf;
      }
 ...
 *
 * One change today: the shadow test is the soft one, at a single tap. A speck is
 * a pixel; it cannot resolve a gradient by itself. But seven thousand of them
 * scattered through the same penumbra each draw a different sample of it, and
 * the field as a whole fades into the shaft's edge instead of stopping at a
 * line. The estimator is per-particle rather than per-pixel and it costs one
 * texture fetch more than the hard test did.
 */
export function makeMoteMaterial() {
  return new THREE.ShaderMaterial({
    defines: { SH_SEARCH: 3, SH_PCF: 1 },
    uniforms: {
      ...froxelReadUniforms(),
      ...shadowUniforms(),
      uGbuf: { value: null },
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        // Modes 1, 8 and 9 are the same code path on purpose. The comparisons
        // this piece makes are between renders that differ by one uniform each —
        // the return term off, the source collapsed back to a point — so nothing
