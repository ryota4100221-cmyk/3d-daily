...
// 🔴 今日の閾値（Day 069）。地は深い青緑一色で、その上に 150 個の細い帯と
// 数百本の髪の毛のような辺が乗るだけ。実測: t=1（辺 255 本）で 438KB、
// t=0（辺 4349 本）で 940KB。空カンバスは DOM の見出しと巨大数字しか
// 残らないので 100KB を大きく下回る。200KB に置いた——前日の 90KB のままだと
// 空カンバスを通してしまうし、400KB だと t=1 の実描画のほうを弾く。
//
// なお今日は virtual-time-budget=400ms でも満尺で描けた（rig の計算は
// 全部 CPU 側で1回、シェーダは1パス）。それでもリトライ梯子は残す。
const MIN_BYTES = Number(process.env.SHOT_MIN || 200) * 1024
