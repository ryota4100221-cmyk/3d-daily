...
/**
 * Day 042 — the same picture, over a footprint that is no longer round.
 *
...
export const LTC_GLSL = /* glsl */ `
  ...
  uniform float uLtc;
  uniform float uMask;
  // uGlass / uGlassMax / uGlassK / uAniso / uTexLight and the ripCell/ripFetch
  // pair come from SHADOW_GLSL, which is always concatenated ahead of this — the
  // source is described in one place, and this year that description grew a
  // texture and then grew a second axis on its pyramid.
  ...
  vec3 glassFetch(vec3 P, mat3 M, vec3 lobe) {
    ...
    float det = max(abs(c0.x * c1.y - c0.y * c1.x), 1e-9);

    // Half-extents of the preimage ellipse's bounding box, in units of uv, then
    // in texels. uGlassK is delta*N, so it is Day 041's constant unchanged — see
    // glass.js for why the same number now means an angle.
    vec2 fp = vec2(length(c1), length(c0)) * (uGlassK / det);
    ...
    // means here. Two isotropic footprints were available: the larger of today's
    // two levels — the circle that *circumscribes* the ellipse, which is what a
    // GPU would pick to avoid aliasing — and yesterday's own scalar,
    // d = distance / sqrt(area), which is nearer the ellipse's geometric mean and
    // therefore aliases a little on the long axis while over-blurring the short.
    //
    // This restores yesterday's, not the conservative one, because the point of
    // the mode is to be the previous build rather than to be a fair isotropic
    // filter. Cell (m, m) of a ripmap is mip level m texel for texel, so with
    // this line the fetch reads the same memory yesterday's read, at the same
    // coordinates, through today's addressing. The only difference between the
    // two renders is the shape of the region being averaged.
    if (uAniso < 0.5) {
      float d = abs(dA) / pow(A2, 0.75);
      lod = vec2(clamp(log2(max(d * uGlassK, 1.0)), 0.0, uGlassMax));
    }

    gLod = lod;
    return mix(vec3(1.0), ripFetch(uv, lod), uTint);
  }
