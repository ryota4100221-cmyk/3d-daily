...
const CELLS = { false: null, true: null }
/**
 * The ripmap, built once per process — and from Day 046 there are two of them,
 * because the day's claim is in the *data*: the masked atlas carries the glass
 * premultiplied by the aperture and the coverage in alpha, the unmasked one is
 * Day 045's, with alpha 1 and the colour drawn through the leads.
 *
 * Both, rather than one with a flag, because every rule from Day 041 to Day 045
 * has to keep reading the texels it actually read. A rule scored against a
 * different image is not the same rule.
 */
export function cells(masked = false) {
  const k = masked ? 'true' : 'false'
  if (!CELLS[k]) CELLS[k] = buildGlass({ mask: masked }).cells
  return CELLS[k]
}

/**
 * Bilinear sample of ripmap cell (m, n), uv in [0,1], clamped like the shader.
 * Four channels since Day 046: the fourth is the coverage, and it is carried
 * rather than divided so that the caller can accumulate several taps before
 * dividing once — a ratio of means, not a mean of ratios.
 */
export function cell(uv, m, n, masked = false) {
  const w = N >> m
  const h = N >> n
  const src = cells(masked)[m][n]
  ...
  for (let j = 0; j < 2; j++) {
    for (let i = 0; i < 2; i++) {
      ...
      const s = (yi * w + xi) * 4
      for (let c = 0; c < 4; c++) out[c] += wgt * src[s + c]
    }
  }
  return out
}
...
export function ripFetch(uv, lodU, lodV, masked = false) {
  ...
  const fu = lu - u0
  const fv = lv - v0
  const a = cell(uv, u0, v0, masked)
  const b = cell(uv, u1, v0, masked)
  const c = cell(uv, u0, v1, masked)
  const d = cell(uv, u1, v1, masked)
  return [0, 1, 2, 3].map(
...
/**
 * Day 046's divide, as the shader has it. The guard returns white — which is the
 * whole window's coverage-weighted mean, because that is what the pyramid is
 * normalised to — for a footprint with no glass in it to average.
 */
export function glassMean(s) {
  const a = s[3]
  const t = Math.min(Math.max(a / 0.02, 0), 1)
  const k = t * t * (3 - 2 * t)
  return [0, 1, 2].map((c) => 1 + k * (s[c] / Math.max(a, 1e-4) - 1))
}
...
export function eigenEllipse(c0, c1, delta, support = 0) {
  ...
  return {
    ...
    // Day 045. |det J| = sqrt(det Q) = sqrt(l1 l2) — the uv -> solid angle
    // stretch at the fetch point, which is the kernel's peak density up to the
    // cosine. Already formed by the eigenproblem above; returned rather than
    // recomputed so the two halves of today's rule cannot disagree.
    det: Math.sqrt(l1 * l2),
  }
}

/**
 * Day 045. The same ellipse, scaled to a measured area.
 *
 * Aspect and orientation are left exactly as they were — this multiplies both
 * semi-axes by one number — so `ab` is the only thing today changes and the
 * comparison against Day 044 is a comparison of sizes and of nothing else.
 */
export function rescale(e, ab) {
  const cur = e.A * e.B
  if (!(cur > 1e-20) || !(ab > 0) || !Number.isFinite(ab)) return e
  const g = Math.sqrt(ab / cur)
  return { A: e.A * g, B: e.B * g, t: e.t, det: e.det }
}
...
export function tapRule(uv, A, B, t, opt = {}) {
  ...
  const masked = !!opt.masked
  const out = [0, 0, 0, 0]
  let wsum = 0
  for (let k = 0; k < n; k++) {
    const s = n > 1 ? (2 * k - (n - 1)) / (n - 1) : 0
    const c = ripFetch([uv[0] + ct * span * s, uv[1] + st * span * s], lu, lv, masked)
    for (let j = 0; j < 4; j++) out[j] += w[k] * c[j]
    wsum += w[k]
  }
  for (let j = 0; j < 4; j++) out[j] /= wsum
  // Day 046: one divide, after the taps rather than inside them. On the unmasked
  // atlas the coverage is 1 everywhere and this is the identity, so every rule
  // from Day 041 on is scored on exactly the arithmetic it was scored on before.
  //
  // `masked: 'raw'` skips the divide and hands back all four channels, which is
  // how the harness reads a footprint's own coverage — the quantity the day is
  // binned by, taken from the fetch that is being scored rather than recomputed
  // from the geometry beside it.
  if (opt.masked === 'raw') return out
  return masked ? glassMean(out) : out.slice(0, 3)
}
