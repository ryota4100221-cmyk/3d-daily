...
/**
 * Day 046 — the pyramid stops averaging the stone.
 *
 * Yesterday's last measurement was the largest number of the week and it was not
 * about filtering at all. Moving the reference from the bounding rectangle to
 * the six panes — from *what the fetch reads* to *what the integral is over* —
 * cost every rule 1.74 points at once (4.44 to 6.18), which is more than four
 * mornings of footprint work put together. Yesterday's NOTES put the diagnosis
 * in one line and the fix at the top of its own list:
 *
 *   > **フェッチは桟を平均し、積分は桟を見ていない。** 直し方は既にアトラスの
 *   > 中にある：level 0 の RGB にペインマスクを掛け、マスクそのものを A に書く。
 *
 * The whole of Day 041's argument for drawing the cartoon *through* the leads
 * still holds — the stone is carried by the polygons, so a black bar painted in
 * the image would darken the answer twice — and it is answered by the same
 * arithmetic that made the ripmap affordable. A box filter is **linear**, so
 * carrying the mask as its own channel filters it alongside the colour:
 *
 *     level 0     rgb = L . m,     a = m          m = the pane coverage, exactly
 *     cell (m,n)  rgb = mean(L.m), a = mean(m)    by construction, both axes
 *     the fetch   rgb / a  =  the mean of L **over the glass inside the
 *                             footprint**, at any level, in one divide.
 *
 * Nothing about the pyramid, the addressing, the ellipse or the taps moves. One
 * channel that was carrying 1.0 for five days starts carrying the aperture, and
 * the fetch that had been averaging 29% stone stops. It is the premultiplied
 * alpha every compositor uses, doing the job it was invented for — an average
 * over a set whose *shape* is finer than the level being read.
 *
 * Two things follow that are worth stating before they surprise someone:
 *
 *   the exposure. Pinning the mean of level 0 pinned the fetch while the fetch
 *                 returned rgb; it does not once the fetch returns rgb/a. What
 *                 has to be white now is the **coverage-weighted** mean, which
 *                 is exactly what the top of the pyramid divides out to. So the
 *                 normalisation moves from the rectangle's mean to the panes',
 *                 which is the mean Day 040 pinned before the image existed.
 *   the empty     a footprint entirely inside a mullion has a = 0 and no answer,
 *   footprint     because there is no glass in it to average. That set is not
 *                 empty at level 0 — the stone is 15 texels wide. The fetch
 *                 falls back to white, which *is* the whole window's mean by the
 *                 line above, and the blend is over a of 0.02 so nothing steps.
 *
 * ?lead=0 rebuilds the atlas the old way — alpha 1 everywhere, colour drawn
 * through the leads, normalised by the rectangle's mean — and since the divide
 * by an alpha of 1 is the identity, that is Day 045's build reading Day 045's
 * texels through today's shader, with no second code path.
 *
 * ---------------------------------------------------------------------------
 * Day 042's header follows.
 *
 * The same image, under a pyramid that is no longer square.
 *
...
/**
 * Day 046 — the six openings, as rectangles in the image's own [-1, 1] frame.
 *
 * Through `tan`, and that is a correction as well as a convention. `glassFetch`
 * builds the quad it reads from `tan(uSrcBound) * uSrcDist` — the opening on its
 * own **plane** — and a pane's edge is at `tan(cx +- hx)` on that same plane, so
 * this is the frame the fetch's uv actually lives in and the one the harness has
 * been integrating the reference over since yesterday. `measurePaneMean` below
 * used the linear angles instead, which put the pane edges 0.5% out and never
 * mattered while it was a number printed at startup. Today it is the mask.
 */
export const PANE_RECTS = (() => {
  const { rows, cols, half } = WINDOW
  const bx = Math.tan(half[0])
  const by = Math.tan(half[1])
  const out = []
  for (const [cy, hy] of rows) {
    for (const [cx, hx] of cols) {
      out.push({
        u0: Math.tan(cx - hx) / bx,
        u1: Math.tan(cx + hx) / bx,
        v0: Math.tan(cy - hy) / by,
        v1: Math.tan(cy + hy) / by,
      })
    }
  }
  return out
...
/**
 * The exact area of texel (x, y) that is glass, in [0, 1].
 *
 * Analytic rather than supersampled, because this is the one layer of the image
 * that is not a picture: it is an indicator function, level 0 is defined as its
 * box average, and a 2x2 supersample of an indicator can only ever return one of
 * five values per texel. The panes are disjoint axis-aligned rectangles, so the
 * area is a sum of interval overlaps and the answer is exact — which matters
 * because every level above is a box filter *of this*, and a mask that is wrong
 * by a quarter texel at the leads is wrong by a quarter texel eight levels up,
 * where a texel is the whole window.
 */
export function paneCoverage(x, y, N) {
  const u0 = (x / N) * 2 - 1
  const u1 = ((x + 1) / N) * 2 - 1
  const v0 = (y / N) * 2 - 1
  const v1 = ((y + 1) / N) * 2 - 1
  let a = 0
  for (const p of PANE_RECTS) {
    const ou = Math.min(u1, p.u1) - Math.max(u0, p.u0)
    if (ou <= 0) continue
    const ov = Math.min(v1, p.v1) - Math.max(v0, p.v0)
    if (ov <= 0) continue
    a += ou * ov
  }
  return a / ((u1 - u0) * (v1 - v0))
}
 ...
 * caught in a second.
 */
export function renderCartoon(N, masked = true) {
  ...
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      ...
      const i = (y * N + x) * 4
      // Day 046: the alpha channel stops being 1 and starts being the aperture.
      // Premultiplied here and box-filtered by the same two passes as the
      // colour, so every cell of the ripmap holds (mean of L.m, mean of m) and
      // the fetch recovers the mean over the glass with one divide.
      const m = masked ? paneCoverage(x, y, N) : 1
      px[i] = r * 0.25 * m
      px[i + 1] = g * 0.25 * m
      px[i + 2] = b * 0.25 * m
      px[i + 3] = m
  ...
  // Pin the mean to white — but *which* mean is now a consequence of what the
  // fetch returns, not a choice. A 2x1 box preserves the mean on its axis, so
  // pinning a mean here pins it at every level of the pyramid, including the 1x1
  // top a rough surface reads. Before today the fetch returned rgb and the mean
  // to pin was the rectangle's; today it returns rgb/a, whose value at the top
  // is the **coverage-weighted** mean, so that is the one that has to be white.
  //
  // Both are the same two lines with a different weight, and the unmasked build
  // takes the old one — a = 1 everywhere there, so the weighted mean *is* the
  // rectangle's and this is one expression rather than a branch.
  const mean = [0, 0, 0]
  let w = 0
  for (let i = 0; i < N * N; i++) {
    const a = px[i * 4 + 3]
...
/**
 * The two normalisation constants, side by side — the rectangle's mean of the
 * raw cartoon and the coverage-weighted mean of it.
 *
 * Their *ratio* is the only thing today changes about the exposure, and it is
 * needed in two places that would otherwise disagree silently: the NOTES, where
 * it is the honest statement of how far the frame moved, and `lobe-error.mjs`,
 * where the reference is integrated in one build's units and scored against a
 * fetch in the other's. A relative error against a reference in the wrong units
 * would report this constant as an error of the day, in whichever direction
 * happened to flatter it.
 */
export function cartoonMeans(N = GLASS_SIZE) {
  const rect = [0, 0, 0]
  const pane = [0, 0, 0]
  let cov = 0
  for (let y = 0; y < N; y++) {
    for (let x = 0; x < N; x++) {
      const m = paneCoverage(x, y, N)
      let c = [0, 0, 0]
      for (let sy = 0; sy < 2; sy++) {
        for (let sx = 0; sx < 2; sx++) {
          const s = cartoon(((x + 0.25 + sx * 0.5) / N) * 2 - 1, ((y + 0.25 + sy * 0.5) / N) * 2 - 1)
          for (let k = 0; k < 3; k++) c[k] += 0.25 * s[k]
        }
      }
      for (let k = 0; k < 3; k++) {
        rect[k] += c[k]
  ...
  return {
    rect: rect.map((v) => v / (N * N)),
    pane: pane.map((v) => v / (cov || 1)),
    fill: cov / (N * N),
  }
}
 ...
 */
export function buildGlass({ mask = true } = {}) {
  const level0 = renderCartoon(GLASS_SIZE, mask)
  const N = GLASS_SIZE
  const L = GLASS_LEVELS

  // The mean the pyramid is now pinned at, measured back off level 0 rather than
  // asserted: with the mask in alpha this is sum(rgb) / sum(a), which is exactly
  // what cell (8, 8) hands the fetch after the divide. It should read 1 to
  // rounding in the masked build; in the unmasked one it reads the drift between
  // the two conventions, which is the number the exposure moved by today.
  ...
  return { tex, size: N, atlas: A, levels: L, paneMean, cells, mask }
}
...
/**
 * What the top of the pyramid will hand the fetch: sum(rgb) / sum(a) over level
 * 0, which every box level preserves exactly.
 *
 * Day 045 computed this by walking the pane rectangles and counting whole
 * texels, which needed the pane geometry a second time and rounded the leads to
 * the nearest texel in the process. The mask is that geometry, exactly, in the
 * channel next to the colour — so the same number is now a ratio of two sums
 * over the same buffer, and there is no second copy of the window to disagree
 * with the first.
 */
