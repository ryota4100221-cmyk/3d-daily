...
export default function App() {
  const [mode, setMode] = useState(() => {
    ...
    return p != null && p >= 1 && p <= PASSES.length + 9 ? p - 1 : 0
  })
    ...
    <div className="app">
      ...
      <div className="ui">
        <div className="top">
          ...
          <div className="day">Day&nbsp;046 · ★★★★★</div>
        </div>

        <div className="lead">
          <div className="eyebrow">Loop 19 · A mask inside the pyramid</div>
          <h1>
            Leaded<em>.</em>
            <span className="ghost">the&nbsp;stone&nbsp;leaves&nbsp;the&nbsp;average</span>
          </h1>
          <p className="sub">
            Five mornings of footprints, and yesterday the largest measurement of
            the week was not about footprints at all: moving the reference from
            the rectangle the fetch reads to the six panes the integral is over
            cost every rule 1.74 points at once — more than four days of filtering
            put together. The pyramid was averaging the mullions. The integral had
            never been over them.
          </p>
          <p className="sub">
            A box filter is linear, so the aperture can ride in the channel next
            to the colour: level 0 holds the glass premultiplied by its own mask,
            and every cell above holds the two means the same filter made. Their
            ratio is the window averaged over the glass alone — one divide, no
            second fetch, at any level. 5.77% to 4.37%. Press <em>l</em> for
            yesterday, <em>m</em> for Day 044, <em>s</em> for Day 043,{' '}
            <em>x</em> for Day 042, <em>a</em> for Day 041.
          </p>
        </div>

        <div className="bottom">
          <div className="meta">
            <span>Area-light filtering · a premultiplied aperture in the pyramid</span>
            <span>The colour is still drawn through the leads · the mask decides who owns it</span>
            <span>l → Day 045, m → Day 044, s → Day 043, x → Day 042, a → Day 041</span>
            <span>Measured against the weighted mean over the panes, not the rectangle</span>
          </div>

          <div className="passes">
            <div className="passes-label">Render pass · a anisotropy · x taps · s support · m area · l mask</div>
            <ul>
              {PASSES.map((p, i) => (
                <li
                  key={p.key}
