...
// 🔴 今日の閾値（Day 080）。この日は DOM オーバーレイ（大きな明朝の3行・
// 本文・実測値の表）が必ず描かれるので、**3Dが1フレームも出ていなくても
// PNG は 204KB になる**。空カンバス＝28KB という過去の目安がそのまま
// 通らない日で、実際 1回目は SHOT_MIN=60 を楽々通した 204KB の
// 「文字だけの絵」だった（readout が全部 "—" で気づいた）。
// 実測：3Dが出た＝379〜394KB／出ていない＝204KB。間を取って 300KB に置く。
