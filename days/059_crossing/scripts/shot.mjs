...
// 🔴 今日の閾値（Day 059）。DOM に濃いグラデーションのヴェールを敷いているので、
// canvas が1画素も描けていなくても PNG は 459KB 出る。実描画は 827KB。
// 既定を 110KB のまま持ってきて、実際に「空カンバスなのに合格」を1枚掴んだ。
// 教訓（Day 042・057 に続き3度目）: 空カンバス判定のバイト閾値は、canvas 以外の
// 高周波成分を足し引きした日には必ず貼り直す。実描画と空カンバスの中間に置く。
// もう一つの安全網: 画面に出しているカウンターが 0000 / 00:00:00 のまま焼き付いて
// いたら、それは rAF が1回も回っていないという証拠になる（今日はこれで気づけた）。
const MIN_BYTES = Number(process.env.SHOT_MIN || 600) * 1024
