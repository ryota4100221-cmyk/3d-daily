...
export function makeCompositeMaterial() {
  return new THREE.ShaderMaterial({
    ...
    defines: { SH_SEARCH: 8, SH_PCF: 48, SH_MASK: 1, LTC_PANES: MAX_PANES },
    uniforms: {
      ...froxelReadUniforms(),
      ...shadowUniforms(),
      ...ltcUniforms(),
    ...
    fragmentShader: /* glsl */ `
      ...
      vec3 direct(vec3 p, vec3 n, vec3 albedo, float rough, vec2 sofs) {
        ...
        float vis = softVisMask(p + n * uNormalBias, sofs, shSlope(n), visP);
        ...
        if (uLtc > 0.5) {
          ...
          float ffD = ltcDiffuse(p, n, V, visP);
          vec3 ffS = ltcSpecular(p, n, V, rough, uF0, visP) * uSpec;
