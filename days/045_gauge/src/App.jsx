...
export default function App() {
  const [mode, setMode] = useState(() => {
    ...
    return p != null && p >= 1 && p <= PASSES.length + 8 ? p - 1 : 0
  })
    ...
    <div className="app">
      ...
      <div className="ui">
        <div className="top">
          ...
          <div className="day">Day&nbsp;045 · ★★★★★</div>
        </div>

        <div className="lead">
          <div className="eyebrow">Loop 18 · The footprint has an area</div>
          <h1>
            Gauge<em>.</em>
            <span className="ghost">measured,&nbsp;not&nbsp;chosen</span>
          </h1>
          <p className="sub">
            Four mornings of footprints, and their size came from two numbers
            nobody measured: an angular radius picked by eye on Day 041, and
            yesterday&rsquo;s support, which is a unit square&rsquo;s covariance.
            Neither of them knows this opening is six panes in three tiers with
            stone between them.
          </p>
          <p className="sub">
            But the kernel has an area, and an area is not a matter of choice.
            Its mass is the scalar the integral is about to multiply by; its peak
            is the lobe&rsquo;s density where the fetch already looks. Mass over
            peak is an area — one profile constant, measured at 1.2, and the
            chosen radius leaves the size entirely. Against a brute-forced mean
            over the panes: 5.77% against 6.18%. Press <em>m</em> for yesterday,{' '}
            <em>s</em> for Day 043, <em>x</em> for Day 042, <em>a</em> for Day 041.
          </p>
        </div>

        <div className="bottom">
          <div className="meta">
            <span>Area-light filtering · the kernel&rsquo;s mass over the kernel&rsquo;s peak</span>
            <span>The area comes off the radiometry · the shape still comes off Q</span>
            <span>m → Day 044, s → Day 043, x → Day 042, a → Day 041, one atlas</span>
            <span>Measured against the weighted mean over the panes, not the rectangle</span>
          </div>

          <div className="passes">
            <div className="passes-label">Render pass · a anisotropy · x taps · s support · m measured area</div>
            <ul>
              {PASSES.map((p, i) => (
                <li
                  key={p.key}
