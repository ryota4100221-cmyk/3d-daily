...
export const SHADOW_GLSL = /* glsl */ `
  ...
  vec3 ripCell(vec2 uv, vec2 lv) {
    ...
    return TEXLOD(uGlass, t / uRipAtlas, 0.0).rgb;
  }
   ...
   */
  vec3 ripFetch(vec2 uv, vec2 lod) {
    vec2 l0 = floor(lod);
    vec2 l1 = min(l0 + 1.0, vec2(uGlassMax));
    vec2 f  = lod - l0;
    vec3 a = ripCell(uv, vec2(l0.x, l0.y));
    vec3 b = ripCell(uv, vec2(l1.x, l0.y));
    vec3 c = ripCell(uv, vec2(l0.x, l1.y));
    vec3 d = ripCell(uv, vec2(l1.x, l1.y));
  ...
  vec3 glassAt(vec2 s) {
    ...
    return ripCell(clamp(uv, 0.0, 1.0), vec2(uGlassSoft));
  }
