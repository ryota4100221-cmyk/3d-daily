...
/**
 * Day 044 — the ellipse itself, which was never the right one.
 *
...
export const LTC_GLSL = /* glsl */ `
   ...
   */
  vec3 glassFetch(vec3 P, mat3 M, vec3 lobe, float ff) {
    ...
    for (int k = 0; k < LTC_TAPS; k++) {
      ...
      acc  += w * ripFetch(clamp(uv + along * s, 0.0, 1.0), lod);
      wsum += w;
    }
    return mix(vec3(1.0), acc / max(wsum, 1e-6), uTint);
  }
  ...
  vec3 ltcDiffuse(vec3 P, vec3 N, vec3 V, float vis[LTC_PANES]) {
    ...
    float ff = ltcClip(f);
    if (uTexLight > 0.5) tint = glassFetch(P, F, f / l, ff);
    return ff * tint;
  }
  ...
  vec3 ltcSpecular(vec3 P, vec3 N, vec3 V, float rough, float f0, float vis[LTC_PANES]) {
    ...
    if (uTexLight > 0.5) tint = glassFetch(P, M, f / l, ff);
