 ...
 */
function footprint(p1, V1, V2, lobe, delta, support, mass = null) {
  const no = crs(V1, V2)
  const A2 = dot(no, no)
...
for (const s of SET) {
  ...
  const alt = [
    ...
    footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, 'size', { ff: s.ff }),
  ]
  let ai = 0
  RULES.forEach(([, f], k) => {
    const g = f ? fp : alt[ai++]
...
console.log(`     GLASS_K     delta        Day 043      + support     Day 045   mean taps`)
for (const K of [48, 64, 96, 128, 160, 192, 256, 320, 448]) {
  ...
  console.log(
    `  ${String(K).padStart(8)}   ${d.toFixed(3).padStart(7)}   ${pct(a, n).padStart(11)}   ` +
      `${pct(b, n).padStart(10)}${pct(c, n).padStart(12)}   ${(taps / n).toFixed(2).padStart(9)}${mark}`
  )
}
...
console.log(`     kappa       RMS %    exact mass   mean taps    A / measured`)
