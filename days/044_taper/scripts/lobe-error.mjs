...
import { WINDOW, LIGHT, lightPosition } from '../src/palette.js'
import { GLASS_K } from '../src/glass.js'
import { TAP_MAX, tapCount, slabWeights, SUPPORT_VAR } from '../src/footprint.js'
import { cells, ripFetch, box, eigenEllipse, tapRule, N } from './rip.mjs'
 ...
 */
function footprint(p1, V1, V2, lobe, delta, support) {
  const no = crs(V1, V2)
  const A2 = dot(no, no)
  ...
  const e = eigenEllipse(c0, c1, delta * N, support ? N / Math.sqrt(3) : 0)
  if (support === 'both') {
  ...
  return { uv, A: e.A, B: e.B, t: e.t, iso }
}
...
const RULES = [
  ['Day 041  one circle', (fp) => ripFetch(fp.uv, lg(fp.iso), lg(fp.iso))],
  ['Day 042  a level pair', (fp) => {
    const [bu, bv] = box(fp.A, fp.B, fp.t)
    return ripFetch(fp.uv, lg(bu), lg(bv))
  }],
...
for (let i = 0; i < NC; i++) {
  ...
  if (l < 1e-9) continue
  SET.push({ ...cf, ...ex, lobe: mul(ex.f, 1 / l), fl: l })
}
...
for (const s of SET) {
  ...
  const alt = [
    footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, 'size'),
    footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, 'both'),
  ]
  let ai = 0
  RULES.forEach(([, f], k) => {
    const g = f ? fp : alt[ai++]
    const c = f ? f(fp) : tapRule(g.uv, g.A, g.B, g.t, { weights: 'area' })
    const e = err(c, s.tint)
    row.s[k] += e * e
  })
}
if (process.env.DIST) {
...
console.log(`     GLASS_K     delta        Day 043      + support   mean taps`)
...
{
  const stat = { a: 0, b: 0, an: 0, n: 0, sa: 0, sb: 0 }
  for (const s of SET) {
    const fp = footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, false)
...
const KNOBS = [
  ['Day 043 (n<=4 area)', { max: TAP_MAX, weights: 'area' }, false],
  ['  + support', { max: TAP_MAX, weights: 'area' }, 'size'],
  ['  + support and centre', { max: TAP_MAX, weights: 'area' }, 'both'],
  ['  + support, gauss', { max: TAP_MAX, weights: 'gauss' }, 'size'],
  ['  + support, flat', { max: TAP_MAX, weights: 'flat' }, 'size'],
  ['  + support, n<=1', { max: 1, weights: 'area' }, 'size'],
  ['  + support, n<=2', { max: 2, weights: 'area' }, 'size'],
  ['  + support, n<=8', { max: 8, weights: 'area' }, 'size'],
  ['  + support, ceil', { max: TAP_MAX, weights: 'area', count: 'ceil' }, 'size'],
]
for (const [label, opt, supp] of KNOBS) {
  let s = 0
  let taps = 0
  let n = 0
  for (const cf of SET) {
    const fp = footprint(cf.p1, cf.V1, cf.V2, cf.lobe, DELTA, supp)
    if (!fp) continue
    const e = err(tapRule(fp.uv, fp.A, fp.B, fp.t, opt), cf.tint)
    s += e * e
...
console.log(`\n  SUPPORT_VAR = ${SUPPORT_VAR}, so the square weighs as an ellipse of N/sqrt(3) = ${(N / Math.sqrt(3)).toFixed(1)} texels`)
