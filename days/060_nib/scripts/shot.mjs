...
// 🔴 今日の閾値（Day 060）。地がほぼ白一色の紙面なので PNG がよく縮む。
// 実測: 実描画 102KB / 空カンバス（DOMだけ残る）29KB。既定の 600KB をそのまま
// 持ってくると毎回リトライ上限まで走って落ちるし、逆に 1KB まで下げると空カンバス
// を掴む（今日1枚掴んだ）。実描画と空カンバスの中間に置く。
// もう一つの安全網: 画面右下の NIB / CONTRAST が「---°」「--%」のまま焼き付いて
// いたら、それは rAF が1回も回っていないという証拠になる。
const MIN_BYTES = Number(process.env.SHOT_MIN || 65) * 1024
