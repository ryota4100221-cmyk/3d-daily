...
// 🔴 今日の閾値（Day 081）。実測：3Dが出た＝185〜211KB／出ていない（DOMの
// 文字だけ）＝66KB。Day 080 から引き継いだ 300KB のままだと**成功した描画を
// 失敗と誤判定して**リトライ階段を最後まで登り切ってしまう（実際そうなった）。
// 閾値は前日から引き継がず、毎日その日の空カンバスと実描画を両方測って置く。
