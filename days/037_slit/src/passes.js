...
export function makeCompositeMaterial() {
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      /**
       * The direct term, from a source with a shape.
       *
       * Three things read uLightAng now, and the early-out at the top is the
       * first place that shows. "dot(n, L) <= 0" used to mean "this face is
       * turned away, it is black, stop" — and for a point source that is exactly
       * true. For a source seventeen degrees across it is not: a face tipped a
       * few degrees past the geometric horizon still sees the top of the
       * opening. So the reject moves out to -sin(theta_max), and the band
       * between there and zero is the terminator, drawn by areaNdL rather than
       * by a clamp.
       *
       * It is a small edit with a large consequence for a frame that has no
       * ambient in it. Everything in that band was previously *black* and lit
       * only by Day 034's wash; it now has a direct term measured in the same
       * units as the rest, and the wash sits underneath it instead of standing
       * in for it.
       *
       * The normal offset before the shadow test stays at 3.5cm — small, and
       * allowed to be small because a 20-tap average over a real penumbra has no
       * step function to shimmer.
       */
      vec3 direct(vec3 p, vec3 n, vec3 albedo, float rough, float rot) {
        vec2 ang = shAng();
        float horizon = -sin(max(ang.x, ang.y));
        ...
        // The cosine knows the source's angular radius *in the direction this
        // surface is rolling*; the lobe knows both angles and which way round
        // they go. They use different directions to the source on purpose — the
        // diffuse term integrates the whole opening, the specular one is pointed
        // at the part of it the mirror direction actually finds.
        float ndlA = areaNdL(n, uLightDir);
        vec3 spec = areaSpec(n, V, uLightDir, rough, uF0) * uSpec;

        vec3 brdf = albedo * (ndlA / PI) + spec;
        return uLightCol * (uLightInt * vis) * brdf;
      }
