...
// 🔴 今日の閾値（Day 071）。地はチャコールの放射グラデーションで、その上に
// 3000m の細線が数千本。**実描画は 476〜491KB**（budget 400〜24000 で毎回同じ桁）。
// 3D が死んで DOM だけ残った場合はグラデーションと文字だけになり、実測で
// 7KB〜200KB 台に落ちる（--disable-3d-apis で 6,992 B を実測）。380KB に置いた。
// 前日の 95KB のままだと空を通し、200KB でも通すので必ず上げ直すこと。
const MIN_BYTES = Number(process.env.SHOT_MIN || 380) * 1024
