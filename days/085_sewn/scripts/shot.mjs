...
// 🔴 今日の閾値（Day 085）。実測：3Dが出た＝74〜86KB／出ていない（DOMの文字だけ）
// ＝55〜64KB。今日の絵はベタ地＋ベタ塗りの軸組で PNG が極端に縮むので、差は20KBしか
// 無い。閾値は72KB。前日の150KBのままだと実描画も全部「空」と判定される。
const MIN_BYTES = Number(process.env.SHOT_MIN || 72) * 1024
