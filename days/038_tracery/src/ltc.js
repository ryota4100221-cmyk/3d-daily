...
export const LTC_GLSL = /* glsl */ `
   ...
   */
  vec3 ltcVector(vec3 P, mat3 M, out float behind) {
    vec3 sum = vec3(0.0);
   ...
   */
  float ltcDiffuse(vec3 P, vec3 N, vec3 V) {
    mat3 F = ltcFrame(N, V);
    float behind;
    vec3 f = ltcVector(P, F, behind);
   ...
   */
  vec3 ltcSpecular(vec3 P, vec3 N, vec3 V, float rough, float f0) {
    ...
    float behind;
    vec3 f = ltcVector(P, Minv * ltcFrame(N, V), behind);
