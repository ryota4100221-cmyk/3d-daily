...
import { TAP_MAX, tapCount, MASS_KAPPA, massArea, erf, tiltCentre, TILT_GAIN } from '../src/footprint.js'
...
function footprint(p1, V1, V2, lobe, delta, ff, support = true, tilt = 0, iter = 0, clip = false) {
  const no = crs(V1, V2)
  const A2 = dot(no, no)
  ...
  // Day 048: the centre, tilted onto the kernel. Last, because the tilt is
  // measured in units of the finished ellipse's own half-lengths.
  let step = 0
  if (tilt > 0) {
    const c = tiltCentre(uv, e.A, e.B, e.t, V1, V2, X, r, N, tilt, iter, clip)
    uv[0] = c[0]
    uv[1] = c[1]
    step = c[2]
  }
  return { uv, A: e.A, B: e.B, t: e.t, step }
}
...
console.log(`\n--- the rule's ellipse, one part at a time replaced by the measured one ---\n`)
{
  ...
  const VARIANTS = [
    ...
    ['+ the tilted centre', (s, f) =>
      footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, TILT_GAIN) || f],
    ['  tilt, then measured shape', (s, f, m) => {
      const g = footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, TILT_GAIN)
      return g ? { uv: g.uv, ...m } : f
    }],
    ['  tilt at gain 1', (s, f) =>
      footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, 1) || f],
  ]
...
// --- 6. today: the tilted centre ---------------------------------------------
//
// The ablation above says the centre is worth 1.13 points and the oracle says
// what the ceiling is. This section is the day's own rule scored the way Day 047
// insisted every rule be scored after the truncation episode: not by the overall
// number, which can be carried by one bin, but by the *distribution* — because a
// mechanism that wins on average and loses wherever the picture is sharpest is
// not an improvement, it is a trade nobody asked for.
//
// Two splits. By visible fraction, which is the blocker's axis and the one this
// harness exists for; and by the footprint's own size in texels, which is the
// axis the tilt should actually vary along — a footprint of two texels has
// nothing to tilt and a footprint covering the opening has everything.
console.log(`\n--- Day 048: the centre, tilted onto the kernel it stands for ---\n`)
{
  const COLS = [
    ['Day 047', 0, 0, false],
    ['tilt 0.5, ships', TILT_GAIN, 0, false],
    ['tilt 1, derived', 1, 0, false],
    ['+ clip, rejected', TILT_GAIN, 0, true],
  ]
  const SIZE_BINS = [4, 16, 64, 1e9]
  const sizeLabel = ['under 4 texels', '4 - 16', '16 - 64', 'over 64 texels']
  const byOpen = OPEN_BINS.map(() => ({ n: 0, s: COLS.map(() => 0) }))
  const bySize = SIZE_BINS.map(() => ({ n: 0, s: COLS.map(() => 0) }))
  const oracle = { open: OPEN_BINS.map(() => 0), size: SIZE_BINS.map(() => 0) }
  let steps = 0
  let moved = 0
  let total = 0
  for (const s of SET) {
    const base = footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff)
    if (!base) continue
    const oi = Math.max(Math.min(OPEN_BINS.findIndex((q) => s.open < q), OPEN_BINS.length - 1), 0)
    const si = Math.max(SIZE_BINS.findIndex((q) => Math.sqrt(base.A * base.B) < q), 0)
    byOpen[oi].n++
    bySize[si].n++
    total++
    const r = ref(s.tintV)
    COLS.forEach(([, g, cv, cl], k) => {
      const fp = g > 0 ? footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, g, cv, cl) : base
      const e = err(tapVis(fp.uv, fp.A, fp.B, fp.t, null), r)
      byOpen[oi].s[k] += e * e
      bySize[si].s[k] += e * e
      if (k === 2) {
        steps += fp.step
        if (fp.step > 0.5) moved++
      }
    })
    const eo = err(tapVis(s.meanV, base.A, base.B, base.t, null), r)
    oracle.open[oi] += eo * eo
    oracle.size[si] += eo * eo
  }
  const head = `${COLS.map(([l]) => l.padStart(16)).join('')}${'measured centre'.padStart(18)}      n`
  const show = (rows, labels, orc) => {
    rows.forEach((r, i) => {
      if (!r.n) return
      console.log(
        `  ${labels[i].padEnd(18)}${r.s.map((x) => pct(x, r.n).padStart(16)).join('')}` +
          `${pct(orc[i], r.n).padStart(18)}   ${String(r.n).padStart(4)}`
      )
    })
    const all = rows.reduce(
      (a, r) => ({ n: a.n + r.n, s: a.s.map((x, k) => x + r.s[k]), o: a.o + 0 }),
      { n: 0, s: COLS.map(() => 0) }
    )
    const ao = orc.reduce((a, b) => a + b, 0)
    console.log(
      `  ${'all'.padEnd(18)}${all.s.map((x) => pct(x, all.n).padStart(16)).join('')}` +
        `${pct(ao, all.n).padStart(18)}   ${String(all.n).padStart(4)}`
    )
  }
  console.log(`  visible fraction  ${head}`)
  show(byOpen, openLabel, oracle.open)
  console.log(`\n  footprint size    ${head}`)
  show(bySize, sizeLabel, oracle.size)
  console.log(
    `\n  mean step ${(steps / total).toFixed(2)} texels; ` +
      `${((100 * moved) / total).toFixed(1)}% of configurations move by more than half a texel`
  )
}

// --- 7. the profile constant, swept ------------------------------------------
//
// MASS_KAPPA got this treatment on Day 045 and it mattered: the value guessed
// from the shape of the lobe was wrong and the sweep said so. TILT_GAIN is 1 by
// derivation — the tilted-uniform model with no fudge — so the only interesting
// outcomes are "the sweep agrees" and "the model is wrong by a factor".
{
  console.log(`\n--- the tilt constant, swept ---\n`)
  const GS = [0, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 3]
  const out = []
  for (const g of GS) {
    let e = 0
    let n = 0
    for (const s of SET) {
      const fp = g > 0
        ? footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, g, 0, false)
        : footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff)
      if (!fp) continue
      e += err(tapVis(fp.uv, fp.A, fp.B, fp.t, null), ref(s.tintV)) ** 2
      n++
    }
    out.push([g, pct(e, n)])
  }
  console.log(`  gain   ${out.map(([g]) => String(g).padStart(8)).join('')}`)
  console.log(`  RMS %  ${out.map(([, v]) => v.padStart(8)).join('')}`)
}

// --- 8. is the step in the right direction at all? ---------------------------
//
// The sweep asks "which multiple of my step is least wrong", which is a question
// that has an answer even when the step points somewhere silly. This is the
// prior question: regress the *measured* displacement — the visible kernel's own
// first moment minus the ray's hit point, which is exactly what Day 047 called
// "the measured centre" — onto the one the derivation predicts.
//
//   slope 1, R near 1     the model is right and the sweep should say 1
//   slope k, R near 1     the model is right in shape and wrong by a factor,
//                         and the factor is then a thing to explain
//   R near 0              the model is wrong, and no gain rescues it
console.log(`\n--- the predicted displacement against the measured one ---\n`)
{
  const stat = (pick) => {
    let num = 0
    let den = 0
    ...
    for (const s of SET) {
      const base = footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff)
      const tilted = footprint(s.p1, s.V1, s.V2, s.lobe, DELTA, s.ff, true, pick[0], 0, pick[1])
      if (!base || !tilted) continue
      const tu = (s.meanV[0] - base.uv[0]) * N
      const tv = (s.meanV[1] - base.uv[1]) * N
      const pu = (tilted.uv[0] - base.uv[0]) * N
      const pv = (tilted.uv[1] - base.uv[1]) * N
      num += tu * pu + tv * pv
      den += pu * pu + pv * pv
      tt += tu * tu + tv * tv
      la += Math.hypot(tu, tv)
    ...
    return {
      slope: num / den,
      r: num / Math.sqrt(den * tt),
      la: la / n,
      lp: lp / n,
    }
  }
  for (const [label, ...pick] of [['the tilt', 1, false], ['the clip alone', 0.001, true], ['both', 1, true]]) {
    const q = stat(pick)
    console.log(
      `  ${label.padEnd(14)}slope ${q.slope.toFixed(3)}   R ${q.r.toFixed(3)}   ` +
        `|measured| ${q.la.toFixed(1)} texels   |predicted| ${q.lp.toFixed(1)}`
    )
  }
}
