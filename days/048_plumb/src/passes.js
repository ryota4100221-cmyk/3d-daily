...
export function makeCompositeMaterial() {
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uShowCtr: { value: 0 }, // 1 = how far the tilt moved the fetch's centre
    },
    depthTest: false,
    depthWrite: false,
  ...
  uniform float uShowCtr;
      ...
      void main() {
        ...
        // Day 048's read-out (?pass=21): how far the tilt moved the fetch's
        // centre, in texels, on a log scale because the range is three decades.
        //
        //   black   the ray was already the kernel's centroid. A small lobe far
        //           from the horizon reads a kernel that is flat across its own
        //           footprint, and there is nothing for a tilt to say.
        //   amber   the fetch slid. The brighter the pixel the further Day 041's
        //           ray was from the first moment of the thing it stands for.
        //
        // Drawn from the step the fetch actually took, the way ?pass=18 is, so
        // ?tilt=0 blacks it out. What it should show, if the argument written at
        // glassFetch is right, is the grazing surfaces and the wide diffuse
        // footprints and *not* the highlight: a lobe narrow enough to resolve a
        // quarry reads a kernel that is flat across its own footprint, and a flat
        // kernel has no first moment to correct.
        if (uShowCtr > 0.5) {
          float s = clamp(log2(1.0 + gCtr) / 7.0, 0.0, 1.0);
          outColor = vec4(vec3(0.06) + s * vec3(0.86, 0.52, 0.10), 1.0);
          return;
        }
