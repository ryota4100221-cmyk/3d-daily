...
import { TAP_MAX as MAX_TAPS, MASS_KAPPA, TILT_GAIN } from './footprint.js'
...
export default function App() {
  const [mode, setMode] = useState(() => {
    ...
    return p != null && p >= 1 && p <= PASSES.length + 11 ? p - 1 : 0
  })
  ...
  // ?tilt=0 takes today's centre back out and puts the fetch on Day 041's ray
  // again. Same atlas, same taps, same ellipse, same addressing: the only thing
  // that moves between 0 and 0.5 is *where* the n taps are centred, which is one
  // clamp on a vec2. The `c` key toggles it and ?pass=21 is the map of how far
  // it moved each pixel's fetch.
  const [tilt, setTilt] = useState(() => {
    const v = readNum('tilt')
    return v != null && v >= 0 ? v : TILT_GAIN
  })
    ...
    <div className="app">
      ...
      <div className="ui">
        <div className="top">
          ...
          <div className="day">Day&nbsp;048 · ★★★★★</div>
        </div>

        <div className="lead">
          <div className="eyebrow">Loop 21 · Where the fetch reads</div>
          <h1>
            Plumb<em>.</em>
            <span className="ghost">the&nbsp;ray&nbsp;was&nbsp;never&nbsp;the&nbsp;centre</span>
          </h1>
          <p className="sub">
            Seven mornings went into how wide the fetch is and how it leans.
            Where it is <em>centred</em> has not moved since Day 041: the ray
            along the lobe's mean direction, where it meets the glass. Yesterday
            measured what each part of that ellipse is worth, and the centre was
            the largest term left by a factor of two.
          </p>
          <p className="sub">
            The mean direction is the centroid of a <em>solid angle</em>. The
            kernel is that solid angle times a cosine, read on a plane — and
            written on the plane it is just{' '}
            <em>max(X·z, 0) / |X|⁴</em>, whose log-gradient is six dot products of
            vectors the fetch already holds. Lean the slab the taps already
            assume by that gradient and its centroid is a Langevin function, so
            the centre slides to the edge of the footprint and never past it.
            4.37% → 3.15%. Press <em>c</em> to put the ray back, and{' '}
            <em>l</em>, <em>m</em>, <em>s</em>, <em>x</em>, <em>a</em> for the six
            mornings under it.
          </p>
        </div>
        ...
        <div className="bottom">
          <div className="meta">
            <span>Area-light filtering · the day the centre was derived</span>
            <span>4.37% → 3.15% over 1024 configurations · one Langevin, no fetch</span>
            <span>c → Day 041's ray, l → Day 046, m → Day 045, s → Day 044</span>
            <span>?pass=21 is how far the tilt moved each pixel's fetch</span>
          </div>

          <div className="passes">
            <div className="passes-label">Render pass · a anisotropy · x taps · s support · m area · l mask · c centre</div>
            <ul>
              {PASSES.map((p, i) => (
                <li
                  key={p.key}
