...
import { TAP_MAX as MAX_TAPS, TILT_GAIN } from './footprint.js'
...
const MODE_FLAT = 12
const MODE_GREY = 13
// Day 042. Not a comparison but an instrument: the level pair the specular
// fetch chose, drawn as colour. Grey is an isotropic footprint — the only kind
// yesterday could express — and everything that is not grey is a pixel where
// the ellipse has an orientation. URL only (?pass=15).
const MODE_LOD = 14
// Day 043's instrument (?pass=16): how many taps the specular fetch took, how
// anisotropic the footprint was, and how far its major axis lies from a texture
// axis. The third channel is the day — it is the quantity a ripmap has no index
// for, and it is zero everywhere yesterday was already right.
const MODE_TAP = 15
// Day 044's instrument (?pass=17): how much of each semi-axis the window's own
// support took off it. Black is a lobe that fits inside the opening — a fetch
// unchanged from yesterday — and yellow is one that had grown past the picture
// and been reading the flat mean of the whole window without saying so. It is
// very nearly the inverse image of ?pass=16.
const MODE_SUPP = 16
// Day 045's (?pass=18): the factor the measured area moved yesterday's ellipse
// by. Grey is a fetch unchanged, teal one the kernel's own mass tightened, amber
// one it widened. Read it next to ?pass=17 — where that one is yellow this one
// is teal, because a footprint the opening had to clamp is a footprint whose
// size was never being computed.
const MODE_GAIN = 17
// Day 046's (?pass=19): how much of the specular fetch's own footprint was
// glass. Black is a footprint inside the panes, where the divide by coverage is
// the identity and the fetch is Day 045's; amber is the share of yesterday's
// answer that came from stone the integral had already excluded.
const MODE_COV = 18
// Day 047's (?pass=20): how much of the glass inside the specular fetch's own
// footprint this point can see. Black is a fetch in open light — the premultiply
// by the fitted visibility is a constant there and today changes nothing — and
// blue is a footprint straddling a blocker's edge, which is the whole set of
// pixels where Day 046 was averaging panes into a surface that cannot see them.
// It is the complement of ?pass=19 in an exact sense: that one is a property of
// the window and never moves, this one is a property of the point and moves with
// everything in the room.
const MODE_VIS = 19
// Day 048's (?pass=21): how far today's tilt moved the fetch's centre, in
// texels. Black is a centre unchanged from Day 041's ray-plane intersection —
// every pixel whose kernel is flat across its own footprint — and amber is a
// fetch that has slid toward the part of the window that actually weighs
// something. Like ?pass=18 it is drawn from the step the fetch actually took, so
// ?tilt=0 blacks it out — the honest reading of that is "nothing moved", which is
// true, and not "there was nothing to move". Day 047 had to make the opposite
// choice for ?pass=20 because that map is a property of the *window*; this one is
// a property of the rule, and a rule that is switched off has no map.
const MODE_CTR = 20
// No day's claim is a mode, because all of them compose with every one of them:
// ?aniso=0 (or the `a` key) collapses the level pair onto the ripmap's diagonal,
// which is Day 041's square pyramid reading the same memory, and ?tap=1 (or `x`)
// puts the whole ellipse back under one fetch, which is Day 042. ?supp=0 (or `s`)
// removes today's, which is Day 043. They nest: ?supp=0 is yesterday, ?supp=0&tap=1
// the day before, ?supp=0&aniso=0 the day before that — four builds on one atlas,
// and any pass in the list can be looked at through any of them.
...
export default function Pipeline({
  ...
  // Day 046 is the one claim of the week that is not a uniform: the mask lives
  // in the atlas, so removing it rebuilds the atlas. Four milliseconds of
  // JavaScript, and the old texture is disposed rather than orphaned — a
  // half-float 512x512 is 2 MB, and the `l` key is exactly the sort of thing a
  // person presses forty times while comparing two renders.
  const glass = useMemo(() => buildGlass({ mask: lead }), [lead])
  useEffect(() => () => glass.tex.dispose(), [glass])
  ...
  useEffect(() => {
    ...
    for (const m of [injMat, compMat, rig.moteMat]) {
      ...
      // The atlas geometry. Two floats, uploaded once, and between them they are
      // the entire addressing scheme — `ripCell` derives every cell's origin and
      // size from them and nothing else knows how the ripmap is packed.
      u.uRipAtlas.value = glass.atlas
      u.uRipBase.value = glass.size
  ...
  useFrame((st, delta) => {
    ...
    for (const m of [injMat, compMat, rig.moteMat]) {
      ...
      // The day's claim, and it is one uniform because it has to be: the
      // isotropic build must differ from the anisotropic one by the choice of
      // level pair and by nothing else — same atlas, same fetch, same taps.
      u.uAniso.value = aniso ? 1 : 0
      // Today's, and it is one uniform for the same reason: 1 is Day 042's
      // single fetch of the whole ellipse's bounding box, and the only thing
      // that moves between 1 and 4 is how many places along the major axis the
      // same ripmap gets read.
      u.uTapMax.value = aniso ? taps : 1
      // Today's, and it is one uniform for the third morning running: the only
      // thing that moves between 0 and N/sqrt(3) is how long the ellipse is —
      // same atlas, same taps, same addressing, one multiply on two floats.
      u.uSupport.value = support
      // Today's, and it is one uniform for the fourth morning running: the only
      // thing that moves between 0 and 1.2 is how long *both* semi-axes are —
      // same atlas, same taps, same aspect, same addressing.
      u.uMass.value = mass
      // Today's, and it is one uniform for the fifth morning running: the only
      // thing that moves between 0 and 1 is whether each tap of the fetch is
      // weighted by the visibility at its own centre — same atlas, same ellipse,
      // same taps, same level pair. The weight is 1 at ?vfit=0 and the multiply
      // stays, so there is no second path through the fetch.
      u.uVisFit.value = vfit ? 1 : 0
      // Today's, and it is one uniform for the sixth morning running: the only
      // thing that moves between 0 and 0.5 is *where* the taps are centred —
      // same atlas, same ellipse, same aspect, same level pair, same count.
      u.uTilt.value = tilt
    ...
    compMat.uniforms.uShowLod.value = mode === MODE_LOD ? 1 : 0
    compMat.uniforms.uShowTap.value = mode === MODE_TAP ? 1 : 0
    compMat.uniforms.uShowSupp.value = mode === MODE_SUPP ? 1 : 0
    compMat.uniforms.uShowGain.value = mode === MODE_GAIN ? 1 : 0
    compMat.uniforms.uShowCov.value = mode === MODE_COV ? 1 : 0
    compMat.uniforms.uShowVis.value = mode === MODE_VIS ? 1 : 0
    compMat.uniforms.uShowCtr.value = mode === MODE_CTR ? 1 : 0
