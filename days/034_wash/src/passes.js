...
import { LIGHT, MOTES, BOUNCE } from './palette.js'
import { VOLUME_GLSL, PHASE_GLSL, froxelReadUniforms, TETRA } from './froxel.js'

/**
 * Day 034 — the rest of the pipeline.
 *
 *   makeLightDepthMaterial  pass 0: the scene from the light's point of view as
 *                           *linear* distance. Day 031's, unchanged.
 *   makeGBufferMaterial     pass 1: MRT. Two attachments since Day 030 — normal
 *                           + depth, velocity + id — and a third one today:
 *                           albedo. A deferred renderer that never wrote albedo
 *                           was a renderer with nothing deferred about its
 *                           shading; the moment a fullscreen pass has to answer
 *                           "how much of this light does this surface keep", it
 *                           needs the one channel this G-buffer was missing.
 *   makeCompositeMaterial   pass 5, today's work: the same trilinear read of the
 *                           integrated volume as before, plus four taps into the
 *                           *un*-integrated one to ask what the air around each
 *                           surface is doing, and give the surface its share.
 *   makeMoteMaterial        pass 6: 7,000 billboards animated entirely in the
 *                           vertex shader, lit by the same depth map as the air,
 *                           faded softly against the G-buffer, attenuated by one
 *                           read of the volume at each speck's own depth.
...
export function makeGBufferMaterial({ far, matId = 0, instanced = false }) {
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      // so the G-buffer's albedo channel is exact rather than approximate, and
      // the composite can multiply by it without apologising.
...
// ---------------------------------------------------------------------------
// pass 5 — composite, and the day's technique
// ---------------------------------------------------------------------------

/**
 * Two reads of the same grid, for two completely different questions.
 *
 * The old one, unchanged since Day 031: sample the *integrated* volume along
 * this pixel's ray and apply the two-term rule — what is behind the air, dimmed
 * by what the air absorbed, plus what the air scattered toward the eye.
 *
 * The new one: sample the *un*-integrated volume — the raw rate channels the
 * injection pass writes, before the prefix scan turns them into an integral —
 * at four points around the shaded surface, and ask what the air standing there
 * is doing. That is the difference between air the camera looks *through* and
 * air a wall is standing *in*, and it is the reason the two reads use different
 * samplers on different textures rather than one read used twice.
 *
 * The conversion is the same one the diffusion makes between neighbouring cells.
 * A froxel holds sigma_s * L; dividing by its own extinction recovers L, which
 * is a property of the place rather than of how thick the air happens to be
 * there. Average that over four directions, weight by cosine, and what comes
 * back is irradiance — up to the constants that live in BOUNCE.gain.
 *
 * Three details that are not obvious and cost a day each when they are wrong:
 *
 *   - The G-buffer normal is in *view* space (three's normalMatrix is built
 *     against the modelView matrix). The taps are placed in world space. The
 *     rotation part of the camera's world matrix is what carries one to the
 *     other, and forgetting it produces a wash that is beautifully lit from a
 *     direction that rotates with the camera.
 *   - The tap positions are projected with the *unjittered* view-projection.
 *     The volume was built through the clean frustum — it is a property of the
 *     room, not of this frame's sub-pixel offset — so a tap projected through
 *     the jittered matrix lands half a texel out and shimmers.
 *   - The volume does not know surfaces exist. Froxels are filled from the light
 *     and the medium, and nothing in that pass has ever heard of the G-buffer,
 *     so the air on the far side of a panel is just as bright in the grid as the
 *     air in front of it. Stepping BOUNCE.bias along the normal before gathering
 *     is what keeps a thin wall from lighting itself from behind. It is a
 *     mitigation, not a fix; the fix is a visibility term this does not have.
 */
export function makeCompositeMaterial() {
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      uViewProj: { value: new THREE.Matrix4() }, // unjittered
    ...
    fragmentShader: /* glsl */ `
      ...
      // One direction of the hemisphere gather. Written as a function taking
      // inout accumulators and called four times rather than looped, because
      // this shader is GLSL ES 1.00 and a const array indexed by a loop counter
      // is a portability argument nobody needs to have.
      void tap(vec3 p, vec3 n, vec3 t, float jit, inout vec3 E, inout float w) {
        ...
        vec3 q = p + n * uBBias + d * (uBRadius * jit);
        ...
        E += min(cell.rgb / max(cell.a, uBFloor), vec3(uBClamp)) * c;
        w += c;
      }
      ...
      vec3 airIrradiance(vec3 p, vec3 n) {
        ...
        // per pixel and per frame breaks the agreement — no two pixels share a
        // cell boundary for long — and the temporal resolve, which is already
        // integrating the kernel's rotation, integrates this too. Noise the
        // filter can eat, in place of structure it cannot.
        float jit = 1.0 + uBJitter * (hash12(gl_FragCoord.xy + vec2(uSeed, uSeed * 1.61)) - 0.5);

        tap(p, n, vec3( 0.5773503,  0.5773503,  0.5773503), jit, E, w);
        tap(p, n, vec3( 0.5773503, -0.5773503, -0.5773503), jit, E, w);
        tap(p, n, vec3(-0.5773503,  0.5773503, -0.5773503), jit, E, w);
        tap(p, n, vec3(-0.5773503, -0.5773503,  0.5773503), jit, E, w);
      ...
      void main() {
        ...
        if (g.a > 1e-5 && uBounce > 0.0) {
          ...
          // view space -> world. mat3(mat4) is a GLSL ES 3.00 constructor, so
          // the columns are taken by hand.
          mat3 camRot = mat3(uCamWorld[0].xyz, uCamWorld[1].xyz, uCamWorld[2].xyz);
          vec3 n = normalize(camRot * g.rgb);
          bounce = texture2D(uAlb, vUv).rgb * airIrradiance(p, n) * uBounce;
        }

        if (uShow > 0.5) {
          gl_FragColor = vec4(max(bounce, vec3(0.0)), 1.0);
          return;
        }
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        // Modes 1, 8 and 9 are the same code path on purpose. The comparison
        // the piece is making is between three renders that differ by one
        // uniform each — the air bounce off, the air's own second bounce off —
        // so nothing else about how they are assembled is allowed to differ.
        if (uMode < 0.5 || (uMode > 6.5 && uMode < 8.5)) {
          ...
          // tenth of the frame's brightness — which is the correct amount for
          // light that has scattered twice and then been reflected once, and is
          // also why it is worth looking at separately.
