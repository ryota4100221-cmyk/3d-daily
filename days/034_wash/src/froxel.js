 ...
 * with no preferred axis. Exported, because two different gathers now want the
 * same one — the diffusion between froxels, and the hemisphere a surface asks
 * the air about. Both spin it by a fresh rotation every frame and let the
 * temporal filter do the integrating.
 */
export const TETRA = [
...
const tetraArrayGLSL = () =>
  `const vec3 TETRA[4] = vec3[4](\n  ${TETRA.map((d) => `vec3(${d.join(', ')})`).join(
    ',\n  '
  )}\n);`

/**
 * Day 034 — the froxel volume, now read by the surfaces standing in it.
 *
 ...
 *
 * Two things changed today, and neither of them is a pass.
 *
 * The albedo became a vec3. A scalar loop gain can only ever make the air a
 * dimmer version of the colour the lamp already was; three of them let the
 * survival probability depend on wavelength, which is the entire reason deep
 * media shift in hue at all. It is one multiply either way.
 *
 * And the *rate* channels — the medium volume this pass writes, before the scan
 * integrates it — acquired a third reader. Day 032 wrote them for the scan. Day
 * 033 made them their own input, one frame later. Today the composite reads them
 * as well, four taps per lit pixel, to ask what the air around a surface is
 * doing. That is only possible because what a cell stores is a property of a
 * place (radiance per unit length) rather than a property of a camera (an
 * integral along a ray) — the distinction Day 033 needed to hand energy between
 * neighbours turns out to be exactly the distinction a surface needs too.
 */
