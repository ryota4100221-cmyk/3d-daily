...
import { LIGHT, lightPosition, ATLAS, SCAN_STEPS, BOUNCE } from './palette.js'
 ...
 *   1. G-buffer      one scene draw, three attachments now: normal + depth,
 *                    velocity + material id, and albedo.
 *   2. Beauty        the lit opaque scene, half float, ONE sample.
 *   3. Injection     ONE draw over a 2048 x 1152 atlas = 256 x 144 x 64 froxels.
 *                    Direct light plus one Jacobi iteration of the diffusion,
 *                    gathered from the previous frame (Day 033).
 *   4. Scan          six ping-pong passes. Offsets 1, 2, 4, 8, 16, 32.
 *   5. Composite     one trilinear read of the *integrated* volume per pixel, as
 *                    always — and, new today, four taps into the *rate* volume
 *                    around each shaded surface, which is where the air stops
 *                    being something the camera looks through and becomes
 *                    something the room is lit by.
 *   6. Motes         7,000 billboards, one draw, into a layer of their own.
 *   7. TAA           reproject, clip, blend — opaque surfaces only. It is also
 *                    what makes a four-tap irradiance estimate viable: the
 *                    kernel spins every frame and the resolve integrates it.
 *   8. Present       motion blur, the mote layer, the grade.
 *
 * The ordering constraint that matters today: the injection writes the rate
 * volume at stage 3 and the composite reads it at stage 5, so the surfaces are
 * lit by *this* frame's air rather than last frame's. The only thing in the
 * pipeline still reading a previous frame on purpose is the diffusion itself,
 * which is the one place where that is the technique rather than a compromise.
 */
...
// the frame without it, and the frame without the air's own second bounce.
const MODE_WASH = 6
const MODE_NO_BOUNCE = 7
const MODE_SINGLE = 8

/**
 * A fresh orientation every frame, axis and angle both advancing by the golden
 * angle so consecutive frames never repeat one. Two gathers use this — the
...
export default function Pipeline({ rig, mode = 0, freeze = null, phase = 0 }) {
  ...
  const gPick = (o) => {
    ...
    if (!e) {
      ...
      // black and therefore receives no wash, which is right: it is a painted
      // gradient standing twenty metres behind the room, not a surface in it.
  ...
  useFrame((st, delta) => {
    ...
    // in 3D world space rather than a screen-space disc.
    {
      ...
      // the surface gather gets its own walk through the same sequence
    ...
    // ---- pass 5: composite + the air's light on the surfaces --------------
    // Two textures from the same grid: `uVol` is the scanned one, integrated
    // along the eye ray; `uMed` is what the injection actually wrote, before any
    // integration, which is the only form in which a cell means something to
    // somebody standing next to it rather than looking down a ray at it.
    ...
    compMat.uniforms.uBounce.value = mode === MODE_NO_BOUNCE ? 0 : BOUNCE.gain
