...
| 2026-07-07 | 008 | Week2・useFrame基礎/Breathe（★2） | days/008_useframe | モーションを回転・浮遊・呼吸の3チャンネルに分解 / delta累積回転でフレームレート非依存の角速度(elapsedTime依存のDay007から前進) / 位相オフセット正弦で浮遊・呼吸が列を伝うtravelling wave / THREE.MathUtils.dampで登場イージング＆スウェイ(非依存) / 外group=浮遊+呼吸/内mesh=回転の入れ子でモーション非干渉 / meshPhysical共用でテラコッタ球のみclearcoat / 手続きIBLで高キー・オフライン安全 |
